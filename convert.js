...
(() => {
  ...
  function parseRuleFilters(args) {
    if (args.include) {
      const included = new Set(
        args.include.split(",").map((r) => r.trim().toLowerCase()).filter(Boolean)
      );
      return { includedRules: included, excludedRules: /* @__PURE__ */ new Set() };
    }
    const excluded = new Set(
      (args.exclude || "").split(",").map((r) => r.trim().toLowerCase()).filter(Boolean)
    );
    return { includedRules: null, excludedRules: excluded };
  }
  function parsePreferPatterns(args) {
    if (!args.prefer) return null;
    return args.prefer.split(",").map((p) => p.trim()).filter(Boolean);
  }
  ...
  function buildProxyGroups({
    ...
    const infrastructureGroups = /* @__PURE__ */ new Set([
      PROXY_GROUPS.SELECT,
      PROXY_GROUPS.MANUAL,
    ...
    function shouldKeepGroup(groupName) {
      ...
      if (group.proxies) {
        group.proxies = group.proxies.filter(
          (p) => (
            ...
            p === "DIRECT" || p === "REJECT" || p === "REJECT-DROP" || // 保留真实的订阅节点
            allProxyNames && allProxyNames.has(p) || // 保留仍然存在的代理组
  ...
  function parsePreferNodes(config, patterns) {
  ...
  // src/rules.ts
  function extractRuleIdentifiers(rule) {
    const identifiers = [];
  ...
  function shouldKeepRule(rule, includedRules, excludedRules) {
    if (rule.includes("DST-PORT,22") || rule.includes("GEOIP,private") || rule.includes("GEOIP,cn") || rule.includes("MATCH")) {
      return true;
    }
  ...
  function getActiveProxyGroupNames(rules) {
  ...
  var require_main = __commonJS({
    "src/main.ts"() {
      ...
      function main(config) {
        ...
        const activeProxyGroupNames = includedRules !== null || excludedRules.size > 0 ? getActiveProxyGroupNames(finalRules) : null;
